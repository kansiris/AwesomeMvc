...
function documentReady(root) {
    ...
    //show code 
    $('.code').wrap('<div/>').parent().hide().before('<br/>').before($('<span class="shcode">show code</span>').click(function () {
        var btn = $(this);
    ...
    $('#chTheme').change(function () {
        ...
        $('#aweStyle').attr('href', root + "Content/themes/" + theme + "/AwesomeMvc.css?v=75");
        $('#modsStyle').attr('href', root + "Content/themes/" + theme + "/awem.css?v=75");
...
function initMenuSeach() {
    ...
    function botFunc() {
        if (($menu.offset().top + $menu.height() + 50) > ($(window).scrollTop() + $(window).height())) {
            $(window).scrollTop($(window).scrollTop() + 10);
        }
    }

    var sctrl = awem.slist($menu, { sel: '.awe-row', enter: onenter, fcls: 'awe-selected', sc: 'n', topf: topFunc, botf: botFunc });
...
var downloadLinks = [
        ...
        { k: "http://aspnetawesome.com/Download/MinSetupCoreDemo", c: "Min Setup Demo - ASP.net Core (Template Project)" },
        { k: "http://aspnetawesome.com/Download/MvcMinSetupDemo", c: "Min Setup Demo - MVC 5 (Template Project)" },
        { k: "http://aspnetawesome.com/Download/MvcTrial", c: "Trial version binaries for ASP.net Core, MVC 5/4/3" },
