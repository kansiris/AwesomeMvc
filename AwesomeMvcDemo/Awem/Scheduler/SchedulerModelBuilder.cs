...
namespace Omu.Awem.Scheduler
{
    ...
    public class SchedulerModelBuilder
    {
        ...
        private List<Column> GenerateColumnsForViewType(DateTime periodStartClient, DateTime periodEndClient)
        {
            ...
            {
                columns.Add(new Column { Bind = "Date", Group = true });
                columns.Add(new Column { Bind = "Time", Width = 200 });
                columns.Add(new Column { Bind = "Title", Header = "Event" });
                columns.Add(new Column { Bind = "Notes" });
            }
            else
            {
                var index = 0;
        ...
        private DateTime HandleCommand()
        {
            ...
            if (Cmd != null)
            {
                ...
                else
                {
                    ...
                    else if (ViewType == SchedulerView.Week || ViewType == SchedulerView.WorkWeek)
                    {
